...
class ScanResultPolicyTest extends TestCase
{
    private ScanResultPolicy $policy;

    protected function setUp(): void
    {
        parent::setUp();

        $this->policy = new ScanResultPolicy;
    }
    ...
    public function test_create_allowed_for_scan_center(): void
    {
        $user = new User;
        $user->role = User::ROLE_SCAN_CENTER;
    ...
    public function test_update_allowed_for_admin(): void
    {
        ...
        $this->assertTrue($this->policy->update($user, new ScanResult));
    }

    public function test_update_allowed_for_scan_center(): void
    {
        $user = new User;
        $user->role = User::ROLE_SCAN_CENTER;
    ...
    public function test_update_denied_for_doctor(): void
    {
        ...
        $this->assertFalse($this->policy->update($user, new ScanResult));
    }
    ...
    public function test_delete_allowed_for_admin_only(): void
    {
        ...
        $scanCenter = new User;
        $scanCenter->role = User::ROLE_SCAN_CENTER;

        $this->assertTrue($this->policy->delete($admin, new ScanResult));
        $this->assertFalse($this->policy->delete($scanCenter, new ScanResult));
    }
}
